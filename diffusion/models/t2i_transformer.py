...
class ComposerTextToImagePixelMMDiT(ComposerModel):
    """ComposerModel for text to image with a diffusion transformer on pixels.
    ...
    Args:
        ...
        tokenizer (transformers.PreTrainedTokenizer): Tokenizer used for
            text_encoder. For a `CLIPTextModel` this will be the
            `CLIPTokenizer` from HuggingFace transformers.
        num_channels (int): The number of channels in the images. Default: `3`.
        image_mean (Optional[tuple[float]]): The means of the images. If not specified, defaults to
            3 * (0.0,). Default: `None`.
    ...
    def __init__(
        ...
        if image_mean is None:
            self.latent_mean = self.num_channels * (0.0)
        if image_std is None:
            self.latent_std = self.num_channels * (1.0)
    ...
    def _apply(self, fn):
        super(ComposerTextToImagePixelMMDiT, self)._apply(fn)
        self.image_mean = fn(self.image_mean)
        self.image_std = fn(self.image_std)
        return self
    ...
    def encode_image(self, image: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Encode an image tensor into patches."""
        # Scale and patchify the image
        image = (image - self.image_mean) / self.image_std
        image_patches, image_coords = patchify(image, self.patch_size)
        return image_patches, image_coords

    @torch.no_grad()
    def decode_image(self, image_patches: torch.Tensor, image_coords: torch.Tensor) -> torch.Tensor:
        """Unscale and unpatchify the image."""
        # Unpatchify the latents
        images = [unpatchify(image_patches[i], image_coords[i], self.patch_size) for i in range(image_patches.shape[0])]
    ...
    def diffusion_forward_process(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if not self.model.training:
            ...
            timesteps = torch.sigmoid(u).view(-1, 1, 1)
            timesteps = self.timestep_shift * timesteps / (1 + (self.timestep_shift - 1) * timesteps)
        # Then, add the noise to the inputs according to the recitified flow
        noise = torch.randn(*inputs.shape, device=inputs.device, generator=self.rng_generator)
        noised_inputs = (1 - timesteps) * inputs + timesteps * noise
        # Compute the targets, which are the velocities
        targets = noise - inputs
    ...
    def forward(self, batch):
        # Get the inputs
        image, caption, caption_mask = batch[self.image_key], batch[self.caption_key], batch[self.caption_mask_key]
        # Get the image patches and coords
        image_patches, image_coords = self.encode_image(image)
        # Get the text embeddings and their coords
        text_embeddings, text_embeddings_coords, caption_mask, pooled_text_embeddings = self.embed_tokenized_prompts(
            caption, caption_mask)
        # Diffusion forward process
    ...
    def generate(self,
        ...
        # Generate initial noise
        images = torch.randn(text_embeddings.shape[0], self.num_channels, height, width, device=device)
        image_patches, image_coords = patchify(images, self.patch_size)
        ...
        text_embeddings_mask = torch.cat([prompt_mask, neg_prompt_mask], dim=0)
        pooled_embedding = torch.cat([pooled_embedding, pooled_neg_embedding], dim=0)
        image_coords_input = torch.cat([image_coords, image_coords], dim=0)

        # backward diffusion process
        timesteps, delta_t = self.make_sampling_timesteps(num_inference_steps)
        timesteps, delta_t = timesteps.to(device), delta_t.to(device)
        for i, t in tqdm(enumerate(timesteps), disable=not progress_bar):
            image_patches_input = torch.cat([image_patches, image_patches], dim=0)
            # Get the model prediction
            model_out = self.model(image_patches_input,
                                   image_coords_input,
                                   t.unsqueeze(0),
                                   conditioning=text_embeddings,
                                   conditioning_coords=text_embeddings_coords,
            ...
            if rescaled_guidance is not None:
                ...
                pred = pred_rescaled * rescaled_guidance + pred * (1 - rescaled_guidance)
            # Update the latents
            image_patches = image_patches - pred * delta_t[i]
        # Decode the latents
        image = self.decode_image(image_patches, image_coords)
        return image.detach()  # (batch*num_images_per_prompt, channel, h, w)
