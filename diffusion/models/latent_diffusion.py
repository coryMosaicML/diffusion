...
"""Custom latent diffusion Composer model."""
...
class LatentDiffusion(ComposerModel):
    ...
    Args:
        ...
        prediction_type (str): The type of prediction to use. Currently `epsilon`, `v_prediction`, `both`, and `all` are supported.
    ...
    def __init__(
        ...
        if self.prediction_type not in ['epsilon', 'v_prediction', 'both', 'all']:
    ...
    def diffusion_forward_process(self, latents: torch.Tensor):
        ...
        timesteps = self.T_max * torch.rand(latents.shape[0], device=latents.device, generator=self.rng_generator)
        ...
        if self.prediction_type == 'epsilon':
            ...
            # Get the (latents, epsilon) targets
            targets = torch.cat([latents, noise], dim=1)
        elif self.prediction_type == 'all':
            v = -sin_t * latents + cos_t * noise
            targets = torch.cat([latents, noise, v], dim=1)
        else:
            raise ValueError(f'Unrecognized prediction type {self.prediction_type}')
    ...
    def loss(self, outputs, batch):
        ...
        if self.prediction_type == 'all':
            ...
            # Compute v from the outputs
            angle = outputs['timesteps'] * torch.pi / (2 * self.T_max)
    ...
    def update_metric(self, batch, outputs, metric):
        if isinstance(metric, MeanSquaredError):
            if self.prediction_type == 'all':
                targ_latents, targ_noise, _ = outputs['targets'].chunk(3, dim=1)
                reduced_targets = torch.cat([targ_latents, targ_noise], dim=1)
                metric.update(outputs['predictions'], reduced_targets)
            else:
    ...
    def update_latents(self, latents, predictions, t, delta_t):
        ...
        if self.prediction_type == 'epsilon':
            ...
            pred_latents, pred_noise = predictions.chunk(2, dim=1)
            v = -sin_t * pred_latents + cos_t * pred_noise
            latents = latents - delta_t * v
        elif self.prediction_type == 'all':
            pred_latents, pred_noise = predictions.chunk(2, dim=1)
            v = -sin_t * pred_latents + cos_t * pred_noise
            latents = latents - delta_t * v
    ...
    def generate(
        ...
        latents = torch.randn((batch_size, self.autoencoder.config['latent_channels'], latent_height, latent_width),
                              device=device,
