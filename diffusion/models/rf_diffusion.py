...
class RFDiffusion(ComposerModel):
    """Latent Diffusion ComposerModel.

    This is a Latent Diffusion model trained with a rectified flow.
    ...
    def rf_forward(self, latents):
        """Perform forward diffusion using a rectified flow."""
        ...
        return noised_latents, timesteps[:, 0, 0, 0], targets
    ...
    def forward(self, batch):
        ...
        # Perform forward diffusion according to the RF process
        model_inputs, timesteps, targets = self.rf_forward(latents)
    ...
    def generate(
        ...
        Args:
            ...
            input_size_params (torch.FloatTensor of size [Bx2], optional): Size parameters
                ...
                Default: `None`.
            heuns_method (bool): Whether to use Heun's method for the diffusion process. If false,
                will use Euler's method. Default: `False`.
        """
        ...
        for i, t in enumerate(timesteps):
            # First, an Euler step to estimate the latents at the next timestep
            velocity_proposed = self.predict_velocity_from_latents(
                latents=latents,
                timestep=t,
            ...
            if heuns_method and i < len(timesteps) - 1:
                t_new = timesteps[i + 1]
                latents_proposed = latents - velocity_proposed * delta_t
                velocity_refined = self.predict_velocity_from_latents(
                ...
                velocity_refined = velocity_proposed
            # Compute the previous timestep latents
            latents = latents - 0.5 * (velocity_proposed + velocity_refined) * delta_t
