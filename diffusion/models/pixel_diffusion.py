...
class UpsamplingPixelDiffusion(ComposerModel):
    """Upsampling pixel space diffusion model.
    ...
    Args:
        ...
        tokenizer (transformers.PreTrainedTokenizer): Tokenizer used for
            ...
            `CLIPTokenizer` from HuggingFace transformers.
        scheduler (diffusers.SchedulerMixin or diffusion.Scheduler): Scheduler to use for diffusion during training.
        upsampling_factor (int): Upsampling factor to use. Default: `4`.
        upsampling_scheduler (diffusers.SchedulerMixin or diffusion.schedulers.ContinuousTimeScheduler): Scheduler to
            use for upsampling noise augmentation. If `None`, defaults to `scheduler`.
        inference_scheduler (diffusers.SchedulerMixin or diffusion.schedulers.ContinuousTimeScheduler): Scheduler to
            use for diffusion during inference. If `None`, defaults to `scheduler`.
        continuous_time (bool): Whether to use continuous time diffusion. Default: `False`.
        input_key (str):  The name of the inputs in the dataloader batch. Default: `image`.
    ...
    def __init__(self,
        ...
        self.augmentation_scheduler = augmentation_scheduler if augmentation_scheduler is not None else scheduler
        self.inference_scheduler = inference_scheduler if inference_scheduler is not None else scheduler
        self.continuous_time = continuous_time
        self.input_key = input_key
        self.conditioning_key = conditioning_key
    ...
    def forward(self, batch, generator=None):
        ...
        if self.continuous_time:
            timesteps = self.scheduler.t_max * torch.rand(inputs.shape[0], device=inputs.device, generator=generator)
            augmentation_timesteps = self.augmentation_scheduler.t_max * torch.rand(
                inputs.shape[0], device=inputs.device, generator=generator)
        else:
            timesteps = torch.randint(0,
                                      len(self.scheduler), (inputs.shape[0],),
                                      device=inputs.device,
                                      generator=generator)
            augmentation_timesteps = torch.randint(0,
                                                   len(self.augmentation_scheduler), (inputs.shape[0],),
                                                   device=inputs.device,
                                                   generator=generator)
        # Add noise to the inputs (forward diffusion)
        noise = torch.randn_like(inputs)
        ...
        # Create the low resolution conditioning input
        # First, downsample by the upsampling factor
        low_res_conditioning = F.interpolate(inputs, scale_factor=1 / self.upsampling_factor, mode='bilinear')
        # Add the augmentation noise
        augmentation_noise = torch.randn_like(low_res_conditioning)
        low_res_conditioning = self.augmentation_scheduler.add_noise(low_res_conditioning, augmentation_noise,
                                                                     augmentation_timesteps)
        # Then, upsample back to the original size
        low_res_conditioning = F.interpolate(low_res_conditioning, scale_factor=self.upsampling_factor, mode='bilinear')
        # Then, concatenate the conditioning onto the inputs
        noised_inputs = torch.cat([noised_inputs, low_res_conditioning], dim=1)
    ...
    def generate(
        self,
        conditioning_images: torch.FloatTensor,
        prompt: Optional[list] = None,
        negative_prompt: Optional[list] = None,
        tokenized_prompts: Optional[torch.LongTensor] = None,
        tokenized_negative_prompts: Optional[torch.LongTensor] = None,
        ...
        Args:
            conditioning_images (torch.FloatTensor): The images to condition the upsampling on.
            prompt (str or List[str]): The prompt or prompts to guide the image generation.
            negative_prompt (str or List[str]): The prompt or prompts to guide the
                image generation away from. Ignored when not using guidance
                (i.e., ignored if guidance_scale is less than 1).
        ...
        # scale the initial noise by the standard deviation required by the scheduler
        images = images * self.inference_scheduler.init_noise_sigma
        # Upsample the conditioning images to the target size
        conditioning_images = F.interpolate(conditioning_images,
                                            size=(height, width),
                                            mode='bilinear',
                                            align_corners=False)
        ...
        # backward diffusion process
        for t in tqdm(self.inference_scheduler.timesteps, disable=not progress_bar):
            # Concatenate the conditioning images to the generated images
            model_input = torch.cat([images, conditioning_images], dim=1)
            if do_classifier_free_guidance:
