...
class StreamingPatchedLatentImageCaptionDataset(StreamingDataset):
    """Streaming dataset for patched-image-caption pairs.
    ...
    def __getitem__(self, index):
        ...
        # Image
        img = torch.from_numpy(np.frombuffer(sample[self.image_key], dtype=np.float16).copy()).reshape(4, 32,
                                                                                                       32).float()
        # Image transforms
        if self.transform is not None:
            img = self.transform(img)
...
def build_streaming_patched_latent_image_caption_dataloader(
    remote: Union[str, List],
    local: Union[str, List],
    batch_size: int,
    caption_drop_prob: float = 0.0,
    ...
    dataset = StreamingPatchedLatentImageCaptionDataset(
        streams=streams,
        caption_drop_prob=caption_drop_prob,
        patch_size=patch_size,
        max_patches=max_patches,
